...
public abstract class UB_Algorithm implements Base {
    // input data
    ...
    public UB_Algorithm(int[][] train, int[][] test, List<Pair>[] rates, int b, int r, double cp, int k, double threshold) {
        _train = train;
        _test = test;
        ...
        _neighbors = new Neighbor[TEST_ROW][TRAIN_ROW];
        _similarity = new double[TEST_ROW][TRAIN_ROW];
    ...
    protected void calAvgRatesTrainColSmooth() {
        for (int i = 0; i < TRAIN_COL; i++) {
            _avgRatesTrainSmoothCol[i] = _RatesTrainCountCol[i] * _avgRatesTrainCol[i] / (_b + _RatesTrainCountCol[i])
                    + _b * _avgRateTrain / (_b + _RatesTrainCountCol[i]);
        }
    }
    ...
    // sort by decreasing order of Math.abs(similarity)
    protected void sortNeighbor(double[] similarity, int testIndex) {
        for (int i = 0; i < TRAIN_ROW; i++) {
            _neighbors[testIndex][i] = new Neighbor(i, similarity[i]);
        }

        Arrays.sort(_neighbors[testIndex], new Comparator<Neighbor>() {
            @Override
