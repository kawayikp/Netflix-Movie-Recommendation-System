 ...
 * + chose K neighbors 		-> hard to determine
 * 
 * best when choose all neighbors
 */
public class IB_AdjustCosine_best extends IB_Algorithm {
    public IB_AdjustCosine_best(int[][] train, int[][] test, List<Pair>[] rates, int b, int r, double cp, int k, double threshold) {
                ...
                while (k > 0 && u < TRAIN_COL) {
                    ...
                    if (_threshold <= Math.abs(neighborSimilarity)) {
                        if (_test[i][neighborIndex] != 0) {
                            numerator += neighborSimilarity * (_test[i][neighborIndex] - _avgRatesTestSmooth[i]);
